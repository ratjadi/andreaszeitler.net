...
function tfe_get_image($id,$content,$align,$width,$height,$default,$regenerate,$default_src) {
	...
	if(empty($src)){
		// Änderung Andeas Zeitler 28.10.2010 23:04
		// extract image auch von airy album, wenn kein image gefunden
		$wpurl = get_bloginfo("wpurl") . '/';
		preg_match("/airy_wrapper\('(.+)'/", $content, $airy_matches);
		$airy_dir = $airy_matches[1];
	    if (is_dir($airy_dir))
			$verzeichnis_handle = dir($airy_dir);
		if ($verzeichnis_handle) {
			while (false !== ($verzeichnis_eintrag = $verzeichnis_handle->read())) {
				if (!is_dir($verzeichnis_eintrag) && eregi("\.(jpe?g)$",$verzeichnis_eintrag)) {	
					// Änderung Andeas Zeitler 17.11.2010 22:35
					// Das Vorschaubild wird ins AIRY_VERZEICHNIS, nicht ins bilderverzeichnis gelegt
					$src = $wpurl.AIRY_VERZEICHNIS . "txprev_" . $verzeichnis_eintrag;
					$srcorig = $src;
					erzeuge_thumbnail($airy_dir, AIRY_VERZEICHNIS, $verzeichnis_eintrag, $width, $height);
					break;
				}
			}
		}
...
// Änderung Andeas Zeitler 29.10.2010 01:13
// Thumbnails für expert bilder aus einem airy-album erzeugen

	function erzeuge_thumbnail ($verzeichnis, $airy_verzeichnis, $verzeichnis_eintrag, $width, $height) {
		$bildpfad_ganz = $verzeichnis . $verzeichnis_eintrag;
		
		// Erzeuge Thumbnail Namen
		$thumbpfad_ganz = $airy_verzeichnis . "txprev_" . $verzeichnis_eintrag;
		$imgtype=array("jpg","jpeg","gif","png");
		foreach($imgtype as $type){
			$thumbpfad_ganz = str_replace(".$type","-".$width."x".$height.".".$type,$thumbpfad_ganz);
		}
		
		//echo $bildpfad_ganz . " -> " . $thumbpfad_ganz ."<br />\n";

		if ( eregi("\.(jpe?g)$",$bildpfad_ganz) && !file_exists($thumbpfad_ganz) ) {
			...
			$thumbnail = imagecreatetruecolor(get_option("thumbnail_size_w"), get_option("thumbnail_size_h")); // schwarzes Bild mit vorgegebener Größe anlegen
			...
			$sample_breite = (int)($breite / 1.2);
			$sample_hoehe = (int)($hoehe / 1.2);
			...
			imagecopyresampled ($thumbnail, $bild,  0, 0, $x, $y, get_option("thumbnail_size_w"), get_option("thumbnail_size_h"), $sample_breite, $sample_hoehe); 
			
			// Bild-Resource in JPEG-Datei schreiben.
			imagejpeg($thumbnail, $thumbpfad_ganz, 170); 
